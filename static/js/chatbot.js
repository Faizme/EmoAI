...
async function checkTimeBasedReminders() {
    try {
        const response = await fetch('/check_time_reminders');
        const data = await response.json();
        
        if (data.show_reminder && data.reminders && data.reminders.length > 0) {
            data.reminders.forEach(reminder => {
                showTimeReminderPopup(reminder);
            });
        }
    } catch (error) {
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    setInterval(checkTimeBasedReminders, 60000);
});
